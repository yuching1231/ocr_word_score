...
def score(name_of_the_file, account_name, post_path):
    ...
    js_text_path = os.path.join(os.getcwd(), 'comment_data', account_name, name_of_the_file + '.json')
    seg_list = jieba.cut( str(df), cut_all = False, HMM = True)
...
def account(account_path, account_name):  #processing every account in the dir "accounts"
    ...
    js_result_path = os.path.join(os.getcwd(), 'comment_data', account_name, 'result.json')
